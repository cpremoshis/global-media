...
with st.sidebar:
    ...
    if display_type == 'Single':
        ...
        #This is because ABC TV is first in the list and includes subtitles
        #I don't want any users to think the subtitles are generated by me, so another outlet will display first by default
        if languages == "All languages":
            st.session_state['selection'] = st.selectbox("Outlet:", broadcasters_filtered_by_lang, index=6)
        else:
