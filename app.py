...
with st.sidebar:
    ...
    if display_type == 'Single view':
        ...
        st.session_state['languages'] = st.multiselect("Select languages:", broadcasters_df['Language'].unique(), placeholder="All languages")
        st.session_state['countries'] = st.multiselect("Select countries:", broadcasters_df['Country'].unique(), placeholder="All countries")
        ...
        if len(st.session_state['languages']) == 0:
            ...
            for row in broadcasters_df.itertuples():
                if row.Language in st.session_state['languages']:
                    broadcasters_filtered_by_lang.append(row.Name)

        #If no language or country is selected
        if len(st.session_state['languages']) == 0 and len(st.session_state['countries']) == 0:
            for row in broadcasters_df.itertuples():
        ...
        #If some combination of country and language is selected
            ...
            for row in broadcasters_df.itertuples():
                if len(st.session_state['languages']) != 0:
                    if len(st.session_state['countries']) != 0:
                        if row.Language in st.session_state['languages'] and row.Country in st.session_state['countries']:
                            if row.Name not in broadcasters_filtered:
                                broadcasters_filtered.append(row.Name)
                    if len(st.session_state['countries']) == 0:
                        if row.Language in st.session_state['languages']:
                            if row.Name not in broadcasters_filtered:
        ...
        #This 'if' is because ABC TV is first in the broadcasters list and their feed includes subtitles
        #I don't want any users to think the subtitles are generated by me, so another outlet will display first by default
        if len(st.session_state['languages']) == 0 and len(st.session_state['countries']) == 0:
            st.session_state['selection'] = st.selectbox("Outlet:", broadcasters_filtered, index=2)
        else:
            st.session_state['selection'] = st.selectbox("Outlet:", broadcasters_filtered)
